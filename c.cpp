...
private:
    ...
    std::vector<std::atomic<uint64_t>> server_stats;
    ...
    void build_dns_packet(const std::string& domain, const std::string& dns_server, 
        ...
        memcpy(packet.data() + sizeof(ip_header) + sizeof(udp_header), &dns_hdr, sizeof(dns_hdr));
...
public:
    ...
    bool load_dns_servers(const std::string& filename) {
        ...
        // 初始化服务器统计
        server_stats.resize(dns_servers.size());
